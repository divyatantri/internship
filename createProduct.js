...
describe('Create Product', function () {
    ...
    function productFunction(productProduct,done){
        var product=element(by.xpath('//md-select[contains(@aria-label,\'find-category-1\')]'));
        product.click();
        ...
        if(productProduct && product.isPresent() && product.isDisplayed() && productProduct.productName) {
            selectProduct.isPresent().then(function (result) {
                if(result){
                    ...
                    var addProduct=element(by.xpath('//div[contains(@class,\'md-active\')]//span'));
                    sign.isClickable(addProduct,function (error,ele) {
                        if(ele) {
                            ele.click();
                            ...
                            if(productProduct.productHsnCode) {
                                productProduct.productHsnCode.forEach(function (hsncode) {
                                    if(hsncode.hsnSearch) {
                                        hsnCode.clear().then(function () {
                                            ...
                                            var selecthsnCode=element(by.xpath('//div[following-sibling::div[./label[text()=\''+hsncode.Code+'\']]]'));
                                            browser.driver.executeScript("arguments[0].scrollIntoView();", selecthsnCode.getWebElement());
                                             console.log(hsncode.Code);
                                                sign.isClickable(selecthsnCode,function (error,ele) {
                                                    if(ele)
                                                        ele.click();
                                                });
                                ...
                                browser.driver.executeScript("arguments[0].click();", next.getWebElement());
                                next.click();
    ...
    function brandFunction(productBrand,done){
        var brand=element(by.xpath('//md-select[contains(@aria-label,\'find-category-2\')]'));
        brand.click();
        ...
        if(productBrand && brand.isPresent() && brand.isDisplayed() && productBrand.brandName){
            selectBrand.isPresent().then(function (result) {
                if(result){
                    ...
                    var addBrand=element(by.xpath('//div[contains(@class,\'md-active\')]//span'));
    ...
    function addCompound(inventory,inventories,done) {
        ...
        //browser.driver.executeScript("arguments[0].scrollIntoView();", selectfirstUOM.getWebElement());
                    ...
                    if(res){
                        ...
                        if(inventory.UOM.size && !isNaN(inventory.UOM.size)) {
                            ...
                            if(inventory.UOM.secondUOM) {
                                ...
                                //browser.driver.executeScript("arguments[0].scrollIntoView();", selectsecondUOM.getWebElement());
                ...
                if(inventory.UOM.UOMName && inventory.NoofUnits && inventory.mrp && !isNaN(inventory.NoofUnits) && !isNaN(inventory.mrp)){
                    ...
                    selectUOM.isPresent().then(function (res) {
                        if(res){
                            ...
                            if(inventory.type === 'Add Compound UOM') {
                                //console.log('entered compund UOM')
                                var addcompound=element(by.xpath('//div[contains(@class,\'md-active\')]//md-select-menu/md-content/md-option[./div[text()=\'Add Compound UOM\']]'));
                                //div[contains(@class,\'md-active\')]//md-select-menu/md-content/md-option[./div[text()=\'Add Compound UOM\']]
                                addcompound.click();
                            ...
                            else if(inventory.type === 'Add Simple UOM'){
                                console.log('entered simple UOM')
                                var adduom=element(by.xpath('//md-option[./div[text()=\'Add Simple UOM\'] and @aria-hidden=\'false\']'));
                                adduom.click();
